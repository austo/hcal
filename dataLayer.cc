...
// TODO: move these to macros.h and use config JSON for env vars.
  ...
  #define CONNSTRING "host=192.168.1.136 dbname=concerto \
    user=appUser password=cAligul@"
  ...
  #define QUERY_GET_ROOMS "select id, name as room_name, display_color \
  ...
  emap_ptr
  DataLayer::get_event_map(time_t start, time_t end, View v){
  ...
  emap_ptr
  DataLayer::get_event_map(time_t start, time_t end, View v, int venue){
    emap_ptr retval = new map<int, list<Event> >();
    result evts = get_events_for_timespan(start, end, venue);
    populate_emap(evts, retval, v);
    return retval;
  ...
  void
  DataLayer::populate_emap(result& evts, emap_ptr emap, View v) {
    // local adjuster with zero-hour offset
        ...
        case week: {
          ...
          */
          if (first_sunday.is_not_a_date() ||
            p_evt_start.date().year() != current_year ||
            p_evt_start.date() < first_sunday){
            get_week_offset_for_current_year(
              p_evt_start, first_sunday, current_year, wk_offset, index);                       
          }
          else{
            index = 0;
          }                                     

          boost::gregorian::days days_since_new_year =
            p_evt_start.date() - first_sunday;

          int weeks_since_first_sunday = 
            days_since_new_year.days() / 7;

          index = index ? 
            index : weeks_since_first_sunday + wk_offset; 
          cout << "v8 - wk index: " << index <<
            " wk_offset: " << wk_offset << endl; 
          break;
        }
        default:
  ...
  result DataLayer::get_events_for_timespan(
    time_t start, time_t end, int venue){
    ptime p_start = from_time_t(start);
    ptime p_end = from_time_t(end);
    connection c(CONNSTRING);
    work txn(c);
    std::stringstream ss;
    ss << QUERY_GET_EVENTS_FUNC
      << txn.quote(to_simple_string(p_start))
      << COMMA_SPACE << txn.quote(to_simple_string(p_end)) << COMMA_SPACE
  ...
  map<int, Room> DataLayer::get_rooms(){
    ...
    for (row = rooms_result.begin(); row != rooms_result.end(); ++row){
      retval[row[COL_ID].as<int>()] = 
        Room(row[COL_ID].as<int>(), 
          row[COL_ROOM_NAME].as<string>(),
          row[COL_DISP_COLOR].as<string>());
    }
    return retval;
  }

  v8::Handle<v8::Value>
  DataLayer::insert_event(
    time_t start,
    time_t end,
    int room_id,
    int leader_id,
    ...
    ss << QUERY_INSERT_EVENT << txn.quote(desc)
      << COMMA_SPACE << txn.quote(to_simple_string(p_evt_start) + "+00")
      << COMMA_SPACE <<  txn.quote(to_simple_string(p_evt_end) + "+00" )
      << COMMA_SPACE << txn.quote(room_id) << COMMA_SPACE 
      << txn.quote(leader_id) << COMMA_SPACE << txn.quote(recurring)
      << QUERY_CLOSE_PARENS_END;
    ...
    if (evt_id == -1){
      stringstream err_ss;
      err_ss << DL_EX_PREFIX
        << "There is already an event at "
        << to_simple_string(p_evt_start) << " in room " << room_id << ".";
      ...
      err_ss << DL_EX_PREFIX
        << "There is already an event at "
        << to_simple_string(p_evt_start) << " with leader "
        << leader_id << ".";
  ...
  /*
   * TODO: may need to change update function return value to int
   * with discrete values for event not found and failure.
   */
  DataLayer::UpdateStatus
  DataLayer::update_event(
    int evt_id, time_t start, time_t end,
    int room_id, int leader_id, string desc, bool recurring) {
    ...
    try {
      ...
      work txn(c);
      stringstream ss;
      ss << QUERY_UPDATE_EVENT << txn.quote(evt_id)
        << COMMA_SPACE << txn.quote(room_id) << COMMA_SPACE 
        << txn.quote(leader_id) << COMMA_SPACE << txn.quote(recurring)
        << COMMA_SPACE << txn.quote(desc) << COMMA_SPACE 
        << txn.quote(to_simple_string(p_evt_start) + "+00") << COMMA_SPACE 
  ...
  void
  DataLayer::get_week_offset_for_current_year(
    ptime& p_evt_start, boost::gregorian::date& first_sunday,
    int& current_year, int& wk_offset, int& index) {
    ...
    first_day_of_the_week_in_month first_sun(
      boost::date_time::Sunday, boost::date_time::Jan);
    ...
    int tmp_offset =
      (int)first_sunday.day() == 1 ? 1 : 2; //handle 2+ year spans
